...
(defun jcs-makefile-app-template ()
  ...
  (insert "\n### All Source ###\n")
  (insert "# main source\n")
  (insert "MAINSRC := $(wildcard $(MAIN_PATH)/*.asm $(MAIN_PATH)/*.c $(MAIN_PATH)/*.cpp)\n")
  (insert "# asm source\n")
  (insert "ASMSRC  := $(wildcard $(SOURCE_PATH)/*.asm)\n")
  (insert "# c/c++ source\n")
  (insert "GSRC    := $(wildcard $(SOURCE_PATH)/*.c $(SOURCE_PATH)/*.cpp)\n")
  (insert "# static link library source\n")
  (insert "ASRC    := $(wildcard $(SOURCE_PATH)/*.c $(SOURCE_PATH)/*.cpp $(STATIC_LIB_PATH)/*.c $(STATIC_LIB_PATH)/*.cpp)\n")
  (insert "# shared link library source\n")
  (insert "SOSRC   := $(wildcard $(SOURCE_PATH)/*.c $(SOURCE_PATH)/*.cpp $(SHARED_LIB_PATH)/*.c $(SHARED_LIB_PATH)/*.cpp)\n")

  (insert "\n### objs ###\n")
  (insert "# main object file\n")
  (insert "MAINOBJ := $(patsubst %.c,%.o, $(patsubst %.cpp,%.o, $(MAINSRC)))\n")
  (insert "# list of object files\n")
  (insert "OBJS    := $(patsubst %.c,%.o, $(patsubst %.cpp,%.o, $(GSRC)))\n")
  (insert "# .a object files\n")
  (insert "AOBJS   := $(patsubst %.c,%.o, $(patsubst %.cpp,%.o, $(ASRC)))\n")
  (insert "# .so object files\n")
  (insert "SOOBJS  := $(patsubst %.c,%.o, $(patsubst %.cpp,%.o, $(SOSRC)))\n")
